...
// Drop marts db
// db.run("DROP TABLE marts")


// Inser data to marts db
...
db.run(sql, (err) => { if (err) console.log("FAILED"); })

// QUERY MARTIAL ARTS
// sql = `SELECT * FROM marts`
...
// update
// sql = `UPDATE marts SET description = ? WHERE id = ? `;
// db.run(sql, ["new description brah", 1], (err) => { if (err) console.error(err.message); })